...
namespace dashing.net.jobs.Examples
{
    ...
    public class Twitter : IJob
    {
        ...
        private static void SendMessage(object message)
        {
            ...
            {
                var url = $"https://api.twitter.com/1.1/search/tweets.json?q={HttpUtility.UrlEncode(SearchTerm)}";

                client.Headers.Add("Authorization", $"{authentication.TokenType} {authentication.AccessToken}");
